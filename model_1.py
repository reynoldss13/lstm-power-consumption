...
model.add(LSTM(X.shape[1], return_sequences=True, dropout=0.2, input_shape=(X.shape[1], X.shape[2])))
model.add(Dense(X.shape[1], activation='tanh'))
model.add(Dense(X.shape[1]))
model.add(Dense(X.shape[2]))

# Compile the model
sgd = SGD(learning_rate=0.01, momentum=0.8, decay=0.05)
model.compile(loss='mae', optimizer=sgd, metrics=['mae', 'mse', 'accuracy'])
...
fit = model.fit(X_train, y_train, epochs=350, batch_size=500, verbose=2,
...
plt.title('Predicted power consumption - Model I', fontsize=12)
